...
- 转码模式只允许旋转、缩放、补边，不裁切
- 支持控制台日志和文件日志
- 支持 dry-run、覆盖、保留临时文件、自定义输出目录和临时目录
...
- macOS Apple Silicon：打开 `VideoMergingTool-macos-apple-silicon.dmg`，将 `VideoMergingTool.app` 拖入 Applications 后从图标启动。
- macOS Intel：打开 `VideoMergingTool-macos-intel.dmg`，将 `VideoMergingTool.app` 拖入 Applications 后从图标启动。
...
只有视频编码、音频编码、分辨率、帧率、像素格式、方向、rotation 等关键参数一致的文件才会进入同一组。
...
- 推送到 `main` 或 `dev`：自动构建平台产物并上传到 Actions Artifacts。
- 推送版本 tag，例如 `v0.2.11-dev`：自动构建 Release 资产并附加到 GitHub Release。

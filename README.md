...
当前 `dev` 分支版本：`5.3.1`。正式安装包是否已发布，请以 [Releases 页面](https://github.com/yibailiu/VideoMergingTool/releases) 为准。
...
- 视频表格除文件名外会按内容自动调整列宽；可拖动表头的明显手柄自定义宽度，双击恢复自适应，设置会保留到下次打开
